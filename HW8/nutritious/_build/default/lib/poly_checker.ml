...
let rec isInside (v, t) = match t with
  ...
  | TVarCompare v, t | t, TVarCompare v ->
    if isInside (v, t) then
      ...
      | TInt | TBool | TString | TLoc _| TVarCompare _ | TVarWrite _  -> make_subst v t
      | _ -> raise (M.TypeError "Type Error")
    )
...
let rec expansive = function
  ...
  | M.APP (e1, e2) -> expansive e1 || expansive e2
...
let rec w (env : typ_env) (exp : M.exp) : subst * typ =
  ...
  | M.VAR x ->
      (try
         let ts = List.assoc x env in
         let t = instantiate ts in
         (empty_subst, t)
  ...
  | M.LET (M.REC (f, x, e1), e2) ->
      ...
      let ts = generalize (subst_env s2 env') t1 in
      let env'' = (f, ts) :: subst_env s2 env in
  ...
  | M.IF (e1, e2, e3) ->
      ...
      let s1'' = s1' @@ s1 in
      let (s2, t2) = w (subst_env s1'' env) e2 in
      let (s3, t3) = w (subst_env s2 env) e3 in
  ...
  (* | M.WRITE e -> *)
      (* let (s, t) = w env e in
      (s, t) *)
      (* let (s1, tau1) = w env e in
      let beta = TVarWrite (new_var()) in
