...
let rec xcps' exp = 
  ...
  | App (e1, e2) ->
      let v1 = new_name () in
      let v2 = new_name () in
      Fn (k_h, App (xcps' e1, Pair(Fn(v1, App(xcps' e2, Pair(Fn(v2, App(App(Var v1, Var v2), Var k_h)), Snd(Var(k_h))))), Snd(Var(k_h)))))
  | Ifp (e1, e2, e3) ->
    let v1 = new_name () in
    let v2 = new_name () in
    let v3 = new_name () in
  ...
  | Pair (e1, e2) ->
    ...
    Fn (k_h,
      App(xcps' e1,
        Pair(
          Fn(v1,
            App(xcps' e2, Pair(Fn(v2, App(Fst(Var k_h), Pair(Var v1, Var v2))), Snd (Var k_h)))), Snd (Var k_h))))

  | Fst e ->
    let v = new_name () in
    Fn (k_h, App(xcps' e, Pair(Fn(v, App(Fst(Var k_h), Fst(Var v))), Snd (Var k_h))))
  | Snd e ->
    let v = new_name () in
    Fn (k_h, App(xcps' e, Pair(Fn(v, App(Fst(Var k_h), Snd(Var v))), Snd (Var k_h))))
  | Raise e ->
    let v = new_name () in
    Fn (k_h, App (xcps' e, Pair(Fn(v, App (Snd (Var k_h), Var v)), Snd (Var k_h))))
  | Handle (e1, x, e2) -> 
    let v = new_name () in
    Fn (k_h, 
    App(xcps' e1, Pair(Fn(v, App(Fst (Var k_h), Var v)), Fn(x, App(xcps' e2, Var k_h)))))
