 ...
 *
 * Refer to the document http://ropas.snu.ac.kr/~kwang/4190.310/24/hw6.pdf for
...
let rec mark_value v = match v with
  | L loc -> reachable_locs := loc :: !reachable_locs
  | R recs -> List.iter (fun (_, loc) -> mark_value (L loc)) recs
  | _ -> ()

let rec mark_stack stack = List.iter (function
  | V v -> mark_value v
  | P (_, _, env) -> mark_env env
  | M (_, ev) -> mark_evalue ev
) stack

and mark_evalue ev = match ev with
  | Loc l -> reachable_locs := l :: !reachable_locs
  | Proc (_, _, env) -> mark_env env

and mark_env env = List.iter (fun (_, ev) -> mark_evalue ev) env

let mark_cont k = List.iter (fun (_, env) -> mark_env env) k
